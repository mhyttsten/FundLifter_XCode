...
//static string ENV_MHTRACER_LINENO = "MHTRACER_LINENO";
...
static string ENV_MHTRACER_FILTER = "ENV_MHTRACER_FILTER";
...
bool traceIt(std::string fileName, std::string functionName, int iLineNumber) {
   const char* estr = std::getenv(ENV_MHTRACER_FILTER.c_str());
   if (estr == nullptr)  return true;
   std::string s = std::string(estr);
   if (s.size() == 0) return true;
   std::string lineNumber = std::to_string(iLineNumber);
   while (true) {
      std::size_t ioE = s.find(";");
      if (s.size() == 0)  return true;
      std::string c = s.substr(0, ioE);
      std::size_t ioFileName = c.find(":");
      std::string fFileName = c.substr(0, ioFileName);
      std::size_t ioFunctionName = c.find(":", ioFileName+1);
      std::string fFunctionName = c.substr(ioFileName+1, ioFunctionName-ioFileName-1);
      std::string fLineNumber = c.substr(ioFunctionName+1, c.size()-ioFunctionName-1);
      if ((fileName == "*" || fFileName == fileName)
          && (functionName == "*" || functionName == fFunctionName)
          && (lineNumber == "*" || lineNumber == fLineNumber)) {
         return false;
      }
      s = s.substr(ioE+1, s.size()-ioE-1);
   }
   return true;
}
...
int main(int argc, char* argv[]) {
   ...
   // estr = "file1:function1:1;file2:function2:*;file3:*:*;";
   // cout << "1: " << traceIt("file4", "whatever", 10) << endl;
   // if (true) return -1;
...
//   if (true) return -1;
   ...
   if (argc <= 1) {
      ...
      string usage = "Usage: [TRACE | TFBZL | REDUCE --help] <filename> [prefix-if-TRACE]\n";
      if (argc < 2) {
         cerr << usage;
         return 1;
      }
   ...
   string skipFiles[] {
      ...
      "tensorflow/core/tfrt/utils/utils_test.cc",
      "tensorflow/lite/kernels/internal/runtime_shape.h",
      "tensorflow/cc/experimental/libtf/value.h",
      "tensorflow/lite/kernels/shim/tensor_view.h",
      "tensorflow/core/distributed_runtime/coordination/coordination_service_error_util.h"
   };
   ...
   if (mode == MODE_TFBZL) {
      ...
      if (idx_matcher == string::npos) {
         cout << "*********** BAZEL ERROR" << endl;
   ...
   //  else if (isH || isC) {
      ...
      cout << "*** Error, no support for C files" << endl;
      assert(false);
      //    s = "\n#include <stdio.h>\n";
      //    s += "#include <stdlib.h>\n";
      //    s += "#include <string.h>\n";
   ...
   while (idx_current < fin.size()) {
      ...
      // Encountered '(', is this a function definition?
         ...
         if (isH || isCC) {
            ...
            cout << "*** Error no support for C: " << fileName << endl;
            assert(false);
...
string codeGenGenerateClass(CPPIndentWriter& iw, const string& fileName) {
   ...
   iw.println("      // Should we trace of filter?");
   iw.println("      const char* env_filter = std::getenv(" + ENV_MHTRACER_FILTER + ");");
   iw.println("      if (env_filter == nullptr) {");
   iw.println("         return true;");
   iw.println("      }");
   ...
   iw.println("         std::size_t ioE = s.find(\";\");");
   iw.println("         if (sFilter.size() == 0) {");
   iw.println("            return true;");
   iw.println("         }");
   ...
   iw.println("         std::string fLineNumber    = c.substr(ioFunctionName+1, c.size()-ioFunctionName-1);");
   iw.println("         if ((fileName == \"*\" || fFileName == fileName)");
   iw.println("            && (functionName == \"*\" || functionName == fFunctionName)");
   iw.println("            && (sLineNumber == \"*\" || sLineNumber == fLineNumber)) {");
   iw.println("            _filtered = true;")
   iw.println("            return;");
   iw.println("         }");
   iw.println("         sfilter = sfilter.substr(ioE+1, s.size()-ioE-1);");
   iw.println("      }");
   iw.println("");
   iw.println("");
   ...
   iw.println("      }");
   iw.println("");
   iw.println("      char* env_dont_print_pid_dash_tid = std::getenv(\"" + ENV_MHTRACER_DONT_PRINT_PID_DASH_TID + "\");");
...
//    iw.println("      ostr += [\"" + "\"")
   ...
   iw.println("      }");
   iw.println("");
   iw.println("      // Don't update indent if tracing was filtered");
